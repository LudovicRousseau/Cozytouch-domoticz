...
dict_cozytouch_devtypes[
    "chauffe eau"
] = "io:AtlanticDomesticHotWaterProductionxxxxx"  # classe désactivée
dict_cozytouch_devtypes["module fil pilote"] = "io:AtlanticElectricalHeaterIOComponent"
dict_cozytouch_devtypes["bridge cozytouch"] = (
    "internal:PodMiniComponent" or "internal:PodV3Component"
)
dict_cozytouch_devtypes[
    "PAC main control"
...
dict_cozytouch_devtypes[
    "PAC Electrical Energy Consumption"
...
def domoticz_write_device_analog(valeur, idx):
    ...
    myurl = (
        url_domoticz + "command&param=udevice&idx=" + idx + "&nvalue=0&svalue=" + valeur
    )
    req = requests.get(myurl)
    if debug:
        print(
...
def domoticz_create_user_variable(nom_variable, valeur_variable):
    ...
    renvoie l'idx créé
    """
    print("domoticz_create_user_variable:", nom_variable, valeur_variable)
    return None

    # Requete de création de variable
    myurl = (
        url_domoticz
        + "command&param=adduservariable&vname="
        + nom_variable
        + "&vtype=0&vvalue="
        + valeur_variable
    )
    ...
    if req.status_code == 200:
        ...
        if data["status"] == ("ERR"):
            myurl = (
                url_domoticz
                + "command&param=saveuservariable&vname="
                + nom_variable
                + "&vtype=0&vvalue="
                + valeur_variable
            )
...
def domoticz_add_virtual_harware():
    ...
    myurl = (
        url_domoticz
        + "command&param=addhardware&htype=15&port=1&name=Cozytouch_V"
        + str(version)
        + "&enabled=true"
    )
    req = requests.get(myurl)
...
def cozytouch_POST(url_device, name, parametre):
    ...
    # si unicode, on teste si c'est un objet JSON '{}' dans ce cas on ne met pas de double quotes, sinon on applique par défaut
        ...
        isinstance(parametre, str)
        or isinstance(parametre, str)
        and parametre.find("{") == -1
    ):
        parametre = '"' + parametre + '"'
...
def test_exist_cozytouch_domoticz_hw_and_backup_store():
    ...
    try:
        ...
        )  # création d'un hardware de type virtual dans domoticz
        if idx == 0:
        ...
        var_save(
            idx, "save_idx"
        )  # création du fichier de sauvegarde de la configuration Cozytouch, et stockage du numéro du hardware cozytouch
    ...
    # Cas où le fichier est existant et virtual hardware de domoticz inexistant/supprimé
        ...
        print(
            (
                "idx hardware cozytouch dans le fichier de sauvegarde de la configuration : "
                + str(save_idx)
            )
        )
        ...
        myurl = (
            "http://" + domoticz_ip + ":" + domoticz_port + "/json.htm?type=hardware"
        )
        req = requests.get(myurl)  # renvoie la liste du hardware domoticz
        if debug:
            print(
                (
                    "  ".join(("GET-> ", myurl, " : ", str(req.status_code))).encode(
                        "utf-8"
                    )
                )
            )
            ...
            if "result" in data:
                while True:
                    try:
                        ...
                        if a == save_idx and data["result"][y][
                            "Name"
                        ] == "Cozytouch_V" + str(version):
                            reset = False
...
def read_label_from_cozytouch(data, x, oid="none"):
    ...
    if oid == "none":
        label = data["setup"]["devices"][x]["label"]
    ...
    # On cherche le numéro 'oid' correspondant au device pour récupérer le nom
        ...
        # Init variable
        y = 0
        z = data["setup"]["rootPlace"]["subPlaces"]
        while True:
            try:
...
def decouverte_devices():
    ...
    if debug:
        print(
            (
                ...
                + " / version : "
                + str(select["connectivity"]["protocolVersion"])
            )
        )
    ...
    if not save_devices:  # si la liste est vide on passe à la création des devices
        ...
        for a in data["setup"]["devices"]:
            url = a["deviceURL"]
            name = a["controllableName"]
            oid = a["placeOID"]

            if name == dict_cozytouch_devtypes.get(
                "radiateur"
            ):  # on vérifie si le nom du device est connu
                label = read_label_from_cozytouch(data, x, oid)
                liste = ajout_radiateur(
                    save_idx, liste, url, x, label
                ...
                liste = ajout_chauffe_eau(
                    save_idx, liste, url, x, (data["setup"]["rootPlace"]["label"])
                )  # label rootplace
                p += 1

            elif name == dict_cozytouch_devtypes.get("module fil pilote"):
                liste = ajout_module_fil_pilote(
                    save_idx, liste, url, x, read_label_from_cozytouch(data, x, oid)
                )
                p += 1

            elif name == dict_cozytouch_devtypes.get("PAC main control"):
                liste = ajout_PAC_main_control(
                    save_idx, liste, url, x, read_label_from_cozytouch(data, x)
                )
                p += 1

            elif name == dict_cozytouch_devtypes.get("PAC zone control"):
                liste = ajout_PAC_zone_control(
                    save_idx, liste, url, x, read_label_from_cozytouch(data, x)
                )
                p += 1

            elif (
                name == dict_cozytouch_devtypes.get("DHWP_THERM_V3_IO")
                or name == dict_cozytouch_devtypes.get("DHWP_THERM_IO")
                or name == dict_cozytouch_devtypes.get("DHWP_THERM_V2_MURAL_IO")
                or name == dict_cozytouch_devtypes.get("DHWP_THERM_V4_CETHI_IO")
            ):
                liste = Add_DHWP_THERM(
                    save_idx, liste, url, x, (data["setup"]["rootPlace"]["label"]), name
                )  # label sur rootplace
                p += 1
                ...
                liste = ajout_PAC_HeatPump(
                    save_idx, liste, url, x, read_label_from_cozytouch(data, x)
                )
                p += 1

            elif name == dict_cozytouch_devtypes.get("PAC OutsideTemp"):
                liste = ajout_PAC_Outside_Temp(
                    save_idx, liste, url, x, read_label_from_cozytouch(data, x)
                )
                p += 1

            elif name == dict_cozytouch_devtypes.get("PAC InsideTemp"):
                liste = ajout_PAC_Inside_Temp(
                    save_idx, liste, url, x, read_label_from_cozytouch(data, x)
                )
                p += 1

            elif name == dict_cozytouch_devtypes.get(
                "PAC Electrical Energy Consumption"
            ):
                liste = ajout_PAC_Electrical_Energy(
                    save_idx, liste, url, x, read_label_from_cozytouch(data, x)
                )
                p += 1

            elif name == dict_cozytouch_devtypes.get("PAC zone component"):
                liste = ajout_PAC_zone_component(
                    save_idx, liste, url, x, read_label_from_cozytouch(data, x)
                )
                p += 1
                ...
                domoticz_write_log(
                    "Cozytouch : Device avec classe " + name + " inconnu"
                )
...
def ajout_radiateur(idx, liste, url, x, label):
    ...
    myurl = (
        ...
        + ":"
        + domoticz_port
        + "/json.htm?addjvalue=0&addjvalue2=0&customimage=15&description=&idx="
        + radiateur["idx_switch_mode"]
        + "&name="
        + nom_switch
        + "+&options="
        + option
    ...
    radiateur["idx_cons_temp_confort"] = domoticz_add_virtual_device(
        idx, 8, nom_cons_conf
    )
    ...
    radiateur["idx_cons_temp_derogation"] = domoticz_add_virtual_device(
        idx, 8, nom_cons_derog
    )
...
def ajout_chauffe_eau(idx, liste, url, x, label):
    ...
    myurl = (
        ...
        + ":"
        + domoticz_port
        + "/json.htm?type=setused&idx="
        + (chauffe_eau["idx_switch_auto_manu"])
        + "&name="
        + nom_switch
        + "&description=&strparam1=&strparam2=&protected=false&switchtype=18&customimage=15&used=true&addjvalue=0&addjvalue2=0&options="
        + option
    )
    req = requests.get(myurl)
    if debug:
    ...
    myurl = (
        ...
        + domoticz_port
        + "/json.htm?type=setused&idx="
        + (chauffe_eau["idx_on_off"])
        + "&name="
        + nom_switch_on_off
        + "&description=&strparam1=&strparam2=&protected=false&switchtype=0&customimage=15&used=true&addjvalue=0&addjvalue2=0&options="
    )
    req = requests.get(myurl)
    if debug:
        print(
    ...
    chauffe_eau["idx_conso_eau"] = domoticz_add_virtual_device(
        idx, 1004, nom_compteur, option="litres"
    )
    ...
    myurl = (
        ...
        + domoticz_port
        + "/json.htm?type=setused&idx="
        + (chauffe_eau["idx_conso_eau"])
        + "&name="
        + nom_compteur
        + "&description=&switchtype=2&addjvalue=0&used=true&options="
    )
    req = requests.get(myurl)
    if debug:
        print(
    ...
    chauffe_eau["idx_compteur_pompe"] = domoticz_add_virtual_device(
        idx, 113, nom_mesure
    )
    # Personnalisation du switch (Modification du nom des levels et de l'icone)
    ...
    myurl = (
        ...
        + domoticz_port
        + "/json.htm?type=setused&idx="
        + (chauffe_eau["idx_compteur_pompe"])
        + "&name="
        + nom_compteur_pompe
        + "&switchtype=3&addjvalue=0&used=true&options="
        + option
    )
    req = requests.get(myurl)
    if debug:
...
def ajout_PAC_main_control(idx, liste, url, x, label):
    ...
    myurl = (
        ...
        + domoticz_port
        + "/json.htm?type=setused&idx="
        + (PAC_main_control["idx_switch_mode"])
        + "&name="
        + nom_switch
        + "&description=&strparam1=&strparam2=&protected=false&switchtype=18&customimage=7&used=true&addjvalue=0&addjvalue2=0&options="
        + option
    )
    req = requests.get(myurl)
    if debug:
...
def ajout_PAC_zone_control(idx, liste, url, x, label):
    ...
    # Création Mesure température :
    nom_mesure = "T°C " + nom
    PAC_zone_control["idx_mesure_temp"] = domoticz_add_virtual_device(
        idx, 80, nom_mesure
    )
    ...
    myurl = (
        ...
        + domoticz_port
        + "/json.htm?type=setused&idx="
        + (PAC_zone_control["idx_switch_mode"])
        + "&name="
        + nom_switch
        + "&description=&strparam1=&strparam2=&protected=false&switchtype=18&customimage=7&used=true&addjvalue=0&addjvalue2=0&options="
        + option
    ...
    PAC_zone_control["idx_cons_temp_confort_chauffage"] = domoticz_add_virtual_device(
        idx, 8, nom_cons_conf_chauffage
    )

    # Consigne température Confort mode climatisation :
    nom_cons_conf_clim = "Confort rafraich. " + nom
    PAC_zone_control["idx_cons_temp_confort_clim"] = domoticz_add_virtual_device(
        idx, 8, nom_cons_conf_clim
    )

    # Consigne température Eco mode chauffage :
    nom_cons_eco_chauffage = "Eco chauff. " + nom
    PAC_zone_control["idx_cons_temp_eco_chauffage"] = domoticz_add_virtual_device(
        idx, 8, nom_cons_eco_chauffage
    )

    # Consigne température Eco mode clim :
    nom_cons_eco_clim = "Eco rafraich. " + nom
    PAC_zone_control["idx_cons_temp_eco_clim"] = domoticz_add_virtual_device(
        idx, 8, nom_cons_eco_clim
    )

    # Consigne température mode manuel :
    nom_cons_manu = "Manuel " + nom
    PAC_zone_control["idx_cons_temp_manu"] = domoticz_add_virtual_device(
        idx, 8, nom_cons_manu
    )
...
def Add_DHWP_THERM(idx, liste, url, x, label, name):
    ...
    DHWP_THERM["idx_compteur_pompe"] = domoticz_add_virtual_device(
        idx, 113, nom_compteur_pompe
    )
    # Personnalisation du switch (Modification du nom des levels et de l'icone
    ...
    send = requests.get(
        ...
        + domoticz_port
        + "/json.htm?type=setused&idx="
        + (DHWP_THERM["idx_compteur_pompe"])
        + "&name="
        + nom_compteur_pompe
        + "&description=&switchtype=3&addjvalue=0&used=true&options="
        + option
    )

    # Compteur d'énergie (ElectricEnergyConsumptionState)
    nom_compteur = "Energie " + nom
    DHWP_THERM["idx_compteur_energie"] = domoticz_add_virtual_device(
        idx, 113, nom_compteur
    )
    ...
    DHWP_THERM["idx_boost_duration"] = domoticz_add_virtual_device(
        idx, 1002, nom_switch
    )
    # Personnalisation du switch (Modification du nom des levels et de l'icone
    ...
    if name == dict_cozytouch_devtypes.get(
        ...
        DHWP_THERM["idx_temp_measurement"] = domoticz_add_virtual_device(
            idx, 80, widget_name
        )

        # Add Heat Pump Energy Counter (io:PowerHeatPumpState)
        widget_name = "Energy HeatPump " + nom
        DHWP_THERM["idx_energy_counter_heatpump"] = domoticz_add_virtual_device(
            idx, 113, widget_name
        )

        # Add Heat Electrical Energy Counter (io:PowerHeatElectricalState)
        widget_name = "Energy Elec " + nom
        DHWP_THERM["idx_energy_counter_heatelec"] = domoticz_add_virtual_device(
            idx, 113, widget_name
        )
        ...
        DHWP_THERM["idx_water_estimation"] = domoticz_add_virtual_device(
            idx, 113, widget_name
        )
        ...
        send = requests.get(
            ...
            + ":"
            + domoticz_port
            + "/json.htm?addjvalue=0&addjvalue2=0&customimage=2&description=&idx="
            + (DHWP_THERM["idx_water_estimation"])
            + "&name="
            + widget_name
            + "&switchtype=2&addjvalue=0&addjvalue2=0&used=true&options="
        )
...
def ajout_PAC_Electrical_Energy(idx, liste, url, x, label):
    ...
    PAC_Electrical_Energy["idx_compteur_1"] = domoticz_add_virtual_device(
        idx, 113, nom_compteur
    )

    # Création Compteur d'énergie 2 :
    nom_compteur = "Energy 2"
    PAC_Electrical_Energy["idx_compteur_2"] = domoticz_add_virtual_device(
        idx, 113, nom_compteur
    )
...
def ajout_PAC_zone_component(idx, liste, url, x, label):
    ...
    myurl = (
        ...
        + domoticz_port
        + "/json.htm?type=setused&idx="
        + (PAC_zone_component["idx_switch_mode"])
        + "&name="
        + nom_switch
        + "&description=&strparam1=&strparam2=&protected=false&switchtype=18&customimage=7&used=true&addjvalue=0&addjvalue2=0&options="
        + option
    ...
    # Consigne température Confort en mode chauffage : (core:ComfortHeatingTargetTemperatureState)
    nom_cons_conf_chauffage = "Confort chauff. " + nom
    PAC_zone_component["idx_cons_temp_confort_chauffage"] = domoticz_add_virtual_device(
        idx, 8, nom_cons_conf_chauffage
    )

    # Consigne température Eco mode chauffage :
    nom_cons_eco_chauffage = "Eco chauff. " + nom
    PAC_zone_component["idx_cons_temp_eco_chauffage"] = domoticz_add_virtual_device(
        idx, 8, nom_cons_eco_chauffage
    )

    # Consigne température mode manuel :
    nom_cons_manu = "Manuel " + nom
    PAC_zone_component["idx_cons_temp_manu"] = domoticz_add_virtual_device(
        idx, 8, nom_cons_manu
    )

    # Switch selecteur durée absence :
    nom_switch = "Duree absence (jours) " + nom
    PAC_zone_component["idx_away_duration"] = domoticz_add_virtual_device(
        idx, 1002, nom_switch
    )
    # Personnalisation du switch (Modification du nom des levels et de l'icone
    option = "TGV2ZWxOYW1lczowfDF8MnwzfDR8NXw2fDc7TGV2ZWxBY3Rpb25zOnx8fHx8fHw7U2VsZWN0b3JTdHlsZTowO0xldmVsT2ZmSGlkZGVuOmZhbHNl"
    send = requests.get(
        ...
        + domoticz_port
        + "/json.htm?addjvalue=0&addjvalue2=0&customimage=15&description=&idx="
        + (PAC_zone_component["idx_away_duration"])
        + "&name="
        + nom_switch
        + "&options="
        + option
    ...
    PAC_zone_component["idx_derog_duration"] = domoticz_add_virtual_device(
        idx, 1002, nom_switch
    )
    # Personnalisation du switch (Modification du nom des levels et de l'icone
    ...
    send = requests.get(
        ...
        + domoticz_port
        + "/json.htm?addjvalue=0&addjvalue2=0&customimage=15&description=&idx="
        + (PAC_zone_component["idx_derog_duration"])
        + "&name="
        + nom_switch
        + "&options="
        + option
...
def gestion_consigne(
    ...
    cons_device,
    cde_name,
    cons_device_abais_eco=0,
    cons_domoticz_confort=0,
    arrondi=True,
):
    ...
    cons_domoticz_prec = var_restore(
        "save_consigne_" + (nom_device.encode("utf-8")) + idx_cons_domoticz
    )
    ...
    if cons_device_abais_eco > 0:
        ...
        cons_device_eco = (
            cons_device - cons_device_abais_eco
    ...
    if (
        cons_device != cons_domoticz
        and cons_domoticz != cons_domoticz_prec
        and (cons_domoticz_prec > 0 or texte == "derogation")
    ):
        ...
        if cons_device_abais_eco > 0:
            ...
            if cons_domoticz_abais_eco < 2:
                ...
                cons_domoticz = (
                    cons_domoticz - cons_domoticz_abais_eco
                )  # Ecriture de la consigne Domoticz
                domoticz_write_device_analog(
                    cons_domoticz, idx_cons_domoticz
                ...
                domoticz_write_log(
                    ...
                    + " : consigne "
                    + texte
                    + " : consigne doit etre 2°C en dessous de la consigne confort ! "
                )

                print(
                    "consigne abaissement éco Domoticz " + str(cons_domoticz_abais_eco)
                )
            cozytouch_POST(
                url_device, cde_name, cons_domoticz_abais_eco
...
def gestion_switch_selector_domoticz(
    cozytouch_mode_actual,
    url_device,
    nom_device,
    idx_switch_domoticz,
    state_cozytouch_on_off="no",
    command_off_activate=False,
    setting_command_on_off="setOperatingMode",
    setting_parameter_off="standby",
    command_on_activate=False,
    setting_parameter_on="on",
    command_manual_activate=False,
    manual_level=10,
    setting_command_manual="setDerogatedMode",
    setting_parameter_manual_on="on",
    setting_parameter_manual_off="off",
    level_0="0",
    level_10="10",
    level_20="20",
    level_30="30",
    ...
    level_70="70",
    level_80="80",
    setting_command_mode="setting_mode",
    ...
    domoticz_mode_old = var_restore(
        "save_" + str(idx_switch_domoticz), format_str=True
    )  # On demande une initialisation avec 'init'

    if debug:
        print(
            (
                "Fonction comparaison switch selecteur : "
                + nom_device
                + " idx:"
                + idx_switch_domoticz
            )
    ...
    if (
        command_off_activate and state_cozytouch_on_off == setting_parameter_off
    ):  # Device à OFF : Si état lu de cozytouch = état  OFF
    ...
    if cozytouch_mode_actual != domoticz_mode_actual and domoticz_mode_old != "init":
        ...
        if domoticz_mode_actual != domoticz_mode_old:
            ...
            if command_off_activate:
                ...
                if domoticz_switch_actual == level_0:
                    cozytouch_POST(
                        url_device, setting_command_on_off, setting_parameter_off
                    )
                # Envoi de la commande 'ON'
                elif command_on_activate:
                    cozytouch_POST(
                        url_device, setting_command_on_off, setting_parameter_on
                    )
            ...
            if command_manual_activate:
                ...
                if domoticz_switch_actual == manual_level:
                    domoticz_write_log(
                        "Cozytouch - "
                        + nom_device
                        + " : nouveau mode transmis: "
                        + str(domoticz_mode_actual)
                    )
                    var_save(domoticz_mode_actual, ("save_" + str(idx_switch_domoticz)))

                else:  # Envoi de la commande 'Dérogation : off'
                    cozytouch_POST(
                        url_device, setting_command_manual, setting_parameter_manual_off
                    )
        ...
        # le changement de mode vient de cozytouch, on envoie le mode à domoticz
            ...
            domoticz_write_log(
                "Cozytouch - "
                + nom_device
                + " : detection changement mode "
                + str(cozytouch_mode_actual)
            )
            domoticz_write_device_switch_selector(
                domoticz_switch_state_to_send, idx_switch_domoticz
            )
...
def value_by_name(data, device, item):
    ...
    print(
        (
            ...
            + item
            + " for device "
            + data["setup"]["devices"][device]["widget"]
        )
    )
    return None
...
def maj_device(data, name, p, x):
    ...
    if name == dict_cozytouch_devtypes.get("module fil pilote"):
        ...
        gestion_switch_selector_domoticz(
            (value_by_name(data, x, "io:TargetHeatingLevelState")),
            classe.get("url"),
            classe.get("nom"),
            classe.get("idx_switch"),
            state_cozytouch_on_off=((value_by_name(data, x, "core:OnOffState"))),
            command_off_activate=True,
            level_0="off",
            level_10="frostprotection",
            level_20="eco",
            level_30="comfort-2",
            level_40="comfort-1",
            level_50="comfort",
    ...
    if name == dict_cozytouch_devtypes.get("radiateur"):
        ...
        # Switch selecteur mode OFF / Manuel / Auto
        gestion_switch_selector_domoticz(
            value_by_name(data, x, "core:OperatingModeState"),
            classe.get("url"),
            classe.get("nom"),
            classe.get("idx_switch_mode"),
            level_0="standby",
            level_10="basic",
            level_20="internal",
            level_30="Derogation",
            setting_command_mode="setOperatingMode",
        )

        # Mesure température : Device : TemperatureSensor, Parametre 1 : core:TemperatureState
        domoticz_write_device_analog(
            (value_by_name(data, (x + 1), "core:TemperatureState")),
            (classe.get("idx_mesure_temp")),
        )
        ...
        gestion_consigne(
            ...
            classe.get("url"),
            classe.get("nom"),
            classe.get("idx_cons_temp_derogation"),
            value_by_name(data, x, "core:DerogatedTargetTemperatureState"),
        ...
        gestion_consigne(
            ...
            classe.get("url"),
            classe.get("nom"),
            classe.get("idx_cons_temp_confort"),
            value_by_name(data, x, "core:ComfortRoomTemperatureState"),
        ...
        gestion_consigne(
            ...
            classe.get("url"),
            classe.get("nom"),
            classe.get("idx_cons_temp_eco"),
            value_by_name(data, x, "core:ComfortRoomTemperatureState"),
            "setSetpointLoweringTemperatureInProgMode",
            cons_device_abais_eco=value_by_name(
                data, x, "io:SetpointLoweringTemperatureInProgModeState"
            ),  # lecture de la consigne éco appliquée par Cozytouch
            cons_domoticz_confort=domoticz_read_device_analog(
                classe.get("idx_cons_temp_confort")
            ),
        ...
        domoticz_write_device_analog(
            (value_by_name(data, x + 4, "core:ElectricEnergyConsumptionState")),
            (classe.get("idx_compteur")),
        )
    ...
    if name == dict_cozytouch_devtypes.get("chauffe eau"):
        ...
        # Switch on/off
        on_off = value_by_name(data, x, "io:OperatingModeCapabilitiesState")[
            "energyDemandStatus"
        ]
        if on_off == 1:
            on_off = "On"
        ...
        # Switch selecteur :
        # Comparaison avec l'état précédent pour mettre à jour uniquement sur changement (évite de remplir les logs inutilement)
        switch_prec = var_restore(
            "save_switch_" + str(classe.get("idx_switch_auto_manu"))
        )
        if switch_prec != switch:
            domoticz_write_device_switch_selector(
                switch, classe.get("idx_switch_auto_manu")
            )
            var_save(switch, ("save_switch_" + str(classe.get("idx_switch_auto_manu"))))

        # Mesure température :
        domoticz_write_device_analog(
            (value_by_name(data, x, "core:TemperatureState")),
            (classe.get("idx_mesure_temp")),
        )

        # Compteur d'eau chaude restante :
        domoticz_write_device_analog(
            value_by_name(data, x, "core:WaterConsumptionState"),
            (classe.get("idx_conso_eau")),
        )
        ...
        # Compteur d'énergie:
        domoticz_write_device_analog(
            str(Pmoy_1min) + ";" + str(wh_actuel), (classe.get("idx_compteur"))
        )

        # Compteur temps de fonctionnement pompe à chaleur :
        domoticz_write_device_analog(
            (value_by_name(data, x, "io:HeatPumpOperatingTimeState")),
            (classe.get("idx_compteur_pompe")),
        )
    ...
    if name == dict_cozytouch_devtypes.get("PAC main control"):
        ...
        gestion_switch_selector_domoticz(
            ...
            level_30="drying",
            level_40="auto",
            setting_command_mode="setPassAPCOperatingMode",
            special_level="auto",
            special_setting="setHeatingCoolingAutoSwitch",
            special_setting_parameter_on="on",
            special_setting_parameter_off="off",
        )
    ...
    if name == dict_cozytouch_devtypes.get("PAC zone control"):
        ...
        gestion_consigne(
            ...
            classe.get("url"),
            classe.get("nom"),
            classe.get("idx_cons_temp_confort_chauffage"),
            value_by_name(data, x, "core:ComfortHeatingTargetTemperatureState"),
            ("setComfortHeatingTargetTemperature"),
        )
        ...
        gestion_consigne(
            ...
            classe.get("url"),
            classe.get("nom"),
            classe.get("idx_cons_temp_confort_clim"),
            value_by_name(data, x, "core:ComfortCoolingTargetTemperatureState"),
        ...
        gestion_consigne(
            ...
            classe.get("url"),
            classe.get("nom"),
            classe.get("idx_cons_temp_eco_chauffage"),
            value_by_name(data, x, "core:EcoHeatingTargetTemperatureState"),
        ...
        gestion_consigne(
            ...
            classe.get("url"),
            classe.get("nom"),
            classe.get("idx_cons_temp_eco_clim"),
            value_by_name(data, x, "core:EcoCoolingTargetTemperatureState"),
        ...
        if mode_PAC == "heating":
            ...
            gestion_consigne(
                ...
                classe.get("url"),
                classe.get("nom"),
                classe.get("idx_cons_temp_manu"),
                value_by_name(data, x, "core:TargetTemperatureState"),
                setting_consigne_zone,
            )

            # Gestion switch sélecteur :
            return_switch = gestion_switch_selector_domoticz(
                state_mode_zone,
                classe.get("url"),
                classe.get("nom"),
                classe.get("idx_switch_mode"),
                ...
                command_off_activate=True,
                setting_command_on_off=setting_command_on_off_mode_zone,
                setting_parameter_off="off",
                command_on_activate=True,
                setting_parameter_on="on",
                level_0="stop",
                level_10="manu",
                level_20="internalScheduling",
                setting_command_mode=setting_command_mode_zone,
            )
            ...
            if return_switch == 1:
                ...
                cozytouch_POST(
                    classe.get("url"),
                    setting_consigne_zone,
                    domoticz_read_device_analog(classe.get("idx_cons_temp_manu")),
                )
    ...
    if name == dict_cozytouch_devtypes.get("PAC_HeatPump"):
        ...
        # Gestion du sélecteur :
        gestion_switch_selector_domoticz(
            value_by_name(data, x, "io:PassAPCOperatingModeState"),
            classe.get("url"),
            classe.get("nom"),
            classe.get("idx_switch_mode"),
            level_0="stop",
    ...
    if name == dict_cozytouch_devtypes.get("PAC Electrical Energy Consumption"):
        ...
        domoticz_write_device_analog(
            (value_by_name(data, x, "core:ConsumptionTariff1State")),
            (classe.get("idx_compteur_1")),
        )
        # Compteur d'énergie 2 :
        domoticz_write_device_analog(
            (value_by_name(data, x, "core:ConsumptionTariff2State")),
            (classe.get("idx_compteur_2")),
        )
    ...
    if name == dict_cozytouch_devtypes.get("PAC zone component"):
        ...
        return_switch = gestion_switch_selector_domoticz(
            ...
            command_on_activate=False,
            setting_parameter_on="on",
            command_manual_activate=True,
            setting_command_manual="setDerogationOnOffState",
            setting_parameter_manual_on="on",
            setting_parameter_manual_off="off",
            level_0="stop",
            level_10="manu",
    ...
    if (
        ...
        # Etat chauffe on/off
        a = (value_by_name(data, x, "io:OperatingModeCapabilitiesState"))[
            "energyDemandStatus"
        ]
        if a == 1:
            on_off = "On"
        ...
        domoticz_write_device_analog(
            value_by_name(data, x + 1, "core:ElectricEnergyConsumptionState"),
            classe.get("idx_compteur_energie"),
        )
        ...
        return_fonction = gestion_switch_selector_domoticz(
            state_chauffe_eau,
            classe.get("url"),
            classe.get("nom"),
            classe.get("idx_switch_mode"),
            state_cozytouch_on_off=(value_by_name(data, x, "core:OperatingModeState"))[
                "absence"
            ],
            level_0="off",
            level_10="manualEcoInactive",
            level_20="manualEcoActive",
            level_30="autoMode",
            level_40="boost",
            setting_command_mode="setDHWMode",
            command_activate=False,
        )
        ...
        if return_fonction[0] == 1:
            if return_fonction[1] == "off":
                ...
                cozytouch_POST(
                    classe.get("url"),
                    "setCurrentOperatingMode",
                    '{"absence":"off", "relaunch":"off"}',
                )

            if (
                return_fonction[1] == "manualEcoInactive"
                or return_fonction[1] == "manualEcoActive"
                or return_fonction[1] == "autoMode"
            ):
                cozytouch_POST(classe.get("url"), "setDHWMode", return_fonction[1])

        # Switch selecteur durée 'boost'
        gestion_switch_selector_domoticz(
            value_by_name(data, x, "core:BoostModeDurationState"),
            classe.get("url"),
            classe.get("nom"),
            classe.get("idx_boost_duration"),
            level_0=0,
            level_10=1,
            level_20=2,
            level_30=3,
            ...
            level_60=6,
            level_70=7,
            setting_command_mode="setBoostModeDuration",
        )

        # Switch selecteur durée 'absence'
        gestion_switch_selector_domoticz(
            int(value_by_name(data, x, "io:AwayModeDurationState")),
            classe.get("url"),
            classe.get("nom"),
            classe.get("idx_away_duration"),
            level_0=0,
            level_10=1,
            level_20=2,
            level_30=3,
            ...
            level_60=6,
            level_70=7,
            setting_command_mode="setAwayModeDuration",
        )
        ...
        if name == dict_cozytouch_devtypes.get(
            ...
            domoticz_write_device_analog(
                (value_by_name(data, x, "io:MiddleWaterTemperatureState")),
                (classe.get("idx_temp_measurement")),
            )

            # Heat Pump Energy Counter (io:PowerHeatPumpState)
            domoticz_write_device_analog(
                (value_by_name(data, x, "io:PowerHeatPumpState")),
                (classe.get("idx_energy_counter_heatpump")),
            )

            # Heat Electrical Energy Counter (io:PowerHeatElectricalState)
            domoticz_write_device_analog(
                (value_by_name(data, x, "io:PowerHeatElectricalState")),
                (classe.get("idx_energy_counter_heatelec")),
            )

            # Water volume estimation  (core:V40WaterVolumeEstimationState)
            domoticz_write_device_analog(
                (value_by_name(data, x, "core:V40WaterVolumeEstimationState")),
                (classe.get("idx_water_estimation")),
            )
...
print(
    (
        "¤¤¤¤ Demarrage script cozytouch <=> domoticz version "
        + str(version)
        + " (debug :"
        + str(debug)
        + ")"
